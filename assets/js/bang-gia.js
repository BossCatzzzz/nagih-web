...
const PRICE_TIERS = [
    { category: "founder", level: "Hạng 01", title: "Founder", price: "2.800.000đ", description: `Founder & Photographer của NAGIH.\nTrực tiếp thực hiện những booking yêu cầu cao về concept, hình ảnh và trải nghiệm.`, peopleLabel: "thợ", linkText: "Xem thợ →" },
    { category: "ekip1", level: "Hạng 02", title: "Ekip 1", price: "2.500.000đ", description: `Nhóm photographer chủ lực, giàu kinh nghiệm và có phong cách cá nhân rõ nét.`, peopleLabel: "thợ", linkText: "Xem thợ →" },
    { category: "ekip23", level: "Hạng 03", title: "Ekip 2, 3", price: "2.200.000đ", description: `Những photographer trẻ của NAGIH, đa dạng phong cách, bắt trend nhanh và được vận hành theo cùng tiêu chuẩn của ekip.`, peopleLabel: "thợ", linkText: "Xem thợ →" },
    { category: "takecare", level: "Hỗ trợ", title: "Take Care", price: "600.000đ", description: `Nhân sự đồng hành xuyên suốt buổi chụp: hỗ trợ trang phục, tóc, phụ kiện, chỉnh dáng, giữ đồ và các nhu cầu phát sinh.`, peopleLabel: "thợ", linkText: "Xem thợ →" }
];

let photographers = [];

function getRealPhotographers(category) {
    return photographers.filter(p => p && p.profile && !p.placeholder && Array.isArray(p.categories) && p.categories.includes(category));
}
...
function renderPriceCards() {
    ...
    grid.innerHTML = PRICE_TIERS.map(tier => {
        ...
        const countText = count === 0 ? "0 thợ hiện có" : `${count} ${tier.peopleLabel}`;
